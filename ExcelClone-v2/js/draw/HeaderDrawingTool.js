...
export class HeaderDrawingTool {
    ...
    drawHeaderRange(from, to, start, end, drawFn) {
    ...
    drawHighlighedColumnHeader(startCol, endCol, scrollLeft) {
        if (this.grid.SELECTED_COL_HEADER === -1)
            return;
        const draw = (col) => this.drawSingleColumnHeader(col, scrollLeft);
        if (this.grid.IS_SELECTING_HEADER &&
            this.grid.HEADER_SELECTION_TYPE === "column") {
            this.drawHeaderRange(this.grid.HEADER_SELECTION_START_COL, this.grid.HEADER_SELECTION_END_COL, startCol, endCol, draw);
        }
        else if (this.grid.SELECTED_COL_RANGE) {
            this.drawHeaderRange(this.grid.SELECTED_COL_RANGE.startCol, this.grid.SELECTED_COL_RANGE.endCol, startCol, endCol, draw);
        }
        else {
    ...
    drawHighlighedRowHeader(startRow, endRow, scrollTop) {
        if (this.grid.SELECTED_ROW_HEADER === -1)
            return;
        const draw = (row) => this.drawSingleRowHeader(row, scrollTop);
        if (this.grid.IS_SELECTING_HEADER &&
            this.grid.HEADER_SELECTION_TYPE === "row") {
            this.drawHeaderRange(this.grid.HEADER_SELECTION_START_ROW, this.grid.HEADER_SELECTION_END_ROW, startRow, endRow, draw);
        }
        else if (this.grid.SELECTED_ROW_RANGE) {
            this.drawHeaderRange(this.grid.SELECTED_ROW_RANGE.startRow, this.grid.SELECTED_ROW_RANGE.endRow, startRow, endRow, draw);
        }
        else {
    ...
    drawSingleColumnHeader(col, scrollLeft) {
        ...
        const colWidth = this.grid.COL_WIDTHS.get(col) || this.grid.DEFAULT_COL_WIDTH;
    ...
    drawSingleRowHeader(row, scrollTop) {
        var _a;
        const ctx = this.grid.context;
        const rowHeight = (_a = this.grid.ROW_HEIGHTS.get(row)) !== null && _a !== void 0 ? _a : this.grid.DEFAULT_ROW_HEIGHT;
