export class CellRangeDrawingTool {
    ...
    drawSelectedCellBorder(startRow, endRow, startCol, endCol, scrollLeft, scrollTop) {
        ...
        const ctx = this.grid.context;
        const getWidth = (col) => { var _a; return (_a = this.grid.COL_WIDTHS.get(col)) !== null && _a !== void 0 ? _a : this.grid.DEFAULT_COL_WIDTH; };
        const getHeight = (row) => { var _a; return (_a = this.grid.ROW_HEIGHTS.get(row)) !== null && _a !== void 0 ? _a : this.grid.DEFAULT_ROW_HEIGHT; };
        const getPos = this.grid.prefixArrayManager.getCellPosition.bind(this.grid.prefixArrayManager);
        ctx.save();
        ...
        if (sRow === eRow && sCol === eCol) {
            ...
            ctx.fillRect(drawX + width - handleSize + 3, drawY + height - handleSize + 3, handleSize, handleSize);
            ctx.restore();
            return;
        }
        ...
        const handleSize = 6;
        ctx.fillStyle = "#187c44";
        ctx.fillRect(borderX + borderW - handleSize + 2, borderY + borderH - handleSize + 2, handleSize + 1, handleSize + 1);
        ctx.restore();
    }
}
