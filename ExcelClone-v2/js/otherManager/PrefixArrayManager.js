export class PrefixArrayManager {
    ...
    createColPrefixArray(endCol) {
        ...
        for (let i = this.latestEndCol + 1; i <= endCol; i++) {
            this.colPrefixArray.push(this.colPrefixArray[i - 1] +
                ((_a = this.grid.COL_WIDTHS.get(i)) !== null && _a !== void 0 ? _a : this.grid.DEFAULT_COL_WIDTH));
        }
    ...
    updateColumnWidth(colIndex) {
        ...
        const widthDiff = ((_b = this.grid.COL_WIDTHS.get(colIndex)) !== null && _b !== void 0 ? _b : this.grid.DEFAULT_COL_WIDTH) -
            oldWidth;
        for (let i = colIndex + 1; i < this.colPrefixArray.length; i++) {
            this.colPrefixArray[i] += widthDiff;
        }
    }
    updateColumnWidthExecute(colIndex, oldWidth, newWidth) {
        const widthDiff = newWidth - oldWidth;
        for (let i = colIndex + 1; i < this.colPrefixArray.length; i++) {
            this.colPrefixArray[i] += widthDiff;
        }
    }
    updateColumnWidthUndo(colIndex, oldWidth, newWidth) {
        const widthDiff = newWidth - oldWidth;
        for (let i = colIndex + 1; i < this.colPrefixArray.length; i++) {
            this.colPrefixArray[i] -= widthDiff;
        }
    }
    updateRowHeightExecute(rowIndex, oldHeight, newHeight) {
        const heightDiff = newHeight - oldHeight;
        for (let i = rowIndex + 1; i < this.rowPrefixArray.length; i++) {
            this.rowPrefixArray[i] += heightDiff;
        }
    }
    updateRowHeightUndo(rowIndex, oldHeight, newHeight) {
        const heightDiff = newHeight - oldHeight;
        for (let i = rowIndex + 1; i < this.rowPrefixArray.length; i++) {
            this.rowPrefixArray[i] -= heightDiff;
        }
    ...
    updateRowHeight(rowIndex) {
        ...
        const heightDiff = ((_b = this.grid.ROW_HEIGHTS.get(rowIndex)) !== null && _b !== void 0 ? _b : this.grid.DEFAULT_ROW_HEIGHT) -
            oldHeight;
        for (let i = rowIndex + 1; i < this.rowPrefixArray.length; i++) {
            this.rowPrefixArray[i] += heightDiff;
        }
