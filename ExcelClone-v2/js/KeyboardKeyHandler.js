...
export class KeyboardKeyHandler {
    ...
    scrollIntoViewIfNeeded() {
        var _a, _b;
        const { startRow, endRow, startCol, endCol } = this.grid.SELECTED_CELL_RANGE;
        ...
        const cellWidth = (_a = this.grid.COL_WIDTHS.get(selCol)) !== null && _a !== void 0 ? _a : this.grid.DEFAULT_COL_WIDTH;
        let y = this.grid.prefixArrayManager.getRowYPosition(selRow);
        const cellHeight = (_b = this.grid.ROW_HEIGHTS.get(selRow)) !== null && _b !== void 0 ? _b : this.grid.DEFAULT_ROW_HEIGHT;
    ...
    shiftSelectedCell(key) {
        const { startRow, startCol, endCol, endRow } = this.grid.SELECTED_CELL_RANGE;
    ...
    handleShiftAndArrowKeyOperations(key) {
        if (key == "ArrowLeft") {
            this.grid.SELECTED_CELL_RANGE.endCol -= 1;
        }
        else if (key == "ArrowRight" || key == "Tab") {
    ...
    handleTabEnterKeyOperations(key, shiftKey = false) {
        if (shiftKey) {
    ...
    handleBackspaceKeyOperation(input) {
    ...
    handleCharacterKeyOperation(key, input) {
        this.cellDataRemovalStrategy.fullRemoval();
    ...
    handleColKeyboardRangeSelection(event) {
        if (this.grid.HEADER_SELECTION_START_COL === -1 ||
    ...
    handleRowKeyboardRangeSelection(event) {
        // Check if we have a valid row selection started
        if (this.grid.HEADER_SELECTION_START_ROW === -1 ||
