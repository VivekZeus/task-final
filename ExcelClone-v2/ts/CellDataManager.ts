...
export class CellDataManager {
  ...
  showCellInputAtPosition(initialChar:any, input:HTMLInputElement) {
    ...
    const cellY =this.grid.prefixArrayManager.getRowYPosition(row) -scrollTop +((this.grid.ROW_HEIGHTS.get(row)??this.grid.DEFAULT_ROW_HEIGHT) - this.grid.DEFAULT_ROW_HEIGHT);
